...
def ARVI(ID):
    ...
    process_areas = cfg.process_area_naipqq
    ...
    for dir, subdir, files in os.walk(naip_dir):
        for f in files:
            inShapefile = process_areas
            inDriver = ogr.GetDriverByName("ESRI Shapefile")
            inDataSource = inDriver.Open(inShapefile, 0)
            inLayer = inDataSource.GetLayer()
            srs = inLayer.GetSpatialRef()
            defLayer = inLayer.GetLayerDefn()

            filter_ids = inLayer.SetAttributeFilter("PROCESS_ID == '%d'") % ID
            sql_code = "PROCESS_ID == '%d'" % ID
            file_names = []
            for i in inLayer:
                if ID == i.GetField('PROCESS_ID'):
                    file_names.append(i.GetFields('FileName'))
            print(file_names)
            res = [sub[2: -15] for sub in file_names]
            ext = '.tif'
            file_names = [x + ext for x in res]
            name = 'arvi_' + str(f)
            output = '%s/%s' % (out_dir, name)
            ...
            if not os.path.exists(output):
                for i in file_names:
                    ...
                    naip = gdal.Open(os.path.join(dir, file_names[i]))
