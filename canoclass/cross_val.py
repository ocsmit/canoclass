...
def cross_val(training_raster, training_fit_raster, alg, folds=5,
    ...
    ---
    Args:
       training_raster: Path of raster training data
       training_fit_raster: Path of raster to fit training data to
       alg: ET or RF :: Which classifier algorithm to utilize
       folds: 5:: The number of folds to use in the cross validation
    Keyword Args:
       class_parameters: Dict:: Arguments for the classifiers
       ...
       ET: {"n_estimators": 100, "criterion": 'gini', "max_depth": None,
            "min_samples_split": 2, "min_samples_leaf": 1,
            "min_weight_fraction_leaf": 0.0, "max_features": 'auto',
            "max_leaf_nodes": None, "min_impurity_decrease": 0.0,
    ...
    X_train, X_test, y_train, y_test = split_data(training_raster, training_fit_raster)
